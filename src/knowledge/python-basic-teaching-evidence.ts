...
const AUTHORED_TEACHING_EVIDENCE: Record<string, TeachingEvidenceSpec> = {
  ...
  K002: {
    coreFactIds: ["F001", "F002", "F003", "F013"],
    exampleFactIds: [["F001", "F003", "F013"], ["F010", "F011"]],
    objective: { behavior: "apply", description: "创建、读取并更新变量，解释顺序执行时重新赋值后的绑定变化。", factIds: ["F001", "F002", "F003", "F013"] },
    misconception: {
      ...
      factIds: ["F001", "F003", "F013"],
    },
    workedExample: {
      title: "跟踪变量更新",
      problem: "预测 age = 18; age = age + 1 的最终输出。",
      steps: [
        { action: "执行 age = 18。", rationale: "顺序结构先执行第一条赋值语句。", factIds: ["F001", "F002", "F013"] },
        { action: "计算 age + 1 得到 19。", rationale: "右侧表达式读取变量当前引用的数据。", factIds: ["F002"] },
        { action: "把 19 重新赋给 age 并输出。", rationale: "后执行的重新赋值让新值覆盖旧绑定。", factIds: ["F003", "F013"] },
      ],
