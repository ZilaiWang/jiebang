    ...
    const load = async () => {
      ...
      try {
        ...
        if (currentUser && currentPlan) {
          setWorkspace((value) => recordPlanPublicState(value, currentUser.id, currentPlan.id, {
...
function ContentReviewCard({ session }: { session: PublicSessionFixture }) {
  ...
  const debates = review.debates ?? []
  return <article className="agent-collaboration-card"><header><span className="card-head-icon"><ShieldCheck size={18} /></span><div><small>多 Agent 辩论 · 发布门禁</small><h2>{reviewStatusLabel(review.overall_status)}</h2></div></header><div className="agent-collaboration-list">{Object.entries(review.workers).map(([worker, state]) => <article key={worker}><span className={`agent-status status-${state.status}`} /><div><b>{workerLabel(worker)}</b><p>{state.last_error ?? `审核 ${state.review_attempt_no} 次，修复 ${state.repair_attempt_no} 次`}</p></div><em>{state.published ? "published" : state.status}</em></article>)}</div>{debates.length ? <div className="review-debate-log"><b>审核方交叉回应与独立仲裁</b>{debates.map((debate, index) => <details key={`${index}-${debate.arbitration.arbiter_agent}`} open><summary>第 {index + 1} 个产物 · {debate.rounds.length} 轮 · 仲裁：{debate.arbitration.decision}</summary>{debate.rounds.map((round) => <div key={round.round_no}><p>第 {round.round_no} 轮：A/B 独立提出 {round.findings.length} 条意见，互相回应 {round.responses.length} 条。</p>{round.responses.map((response) => <p key={response.finding_id}>[{response.agent}] {response.stance} → {response.target_finding_id}：{response.message}</p>)}</div>)}<p><b>独立仲裁：</b>{debate.arbitration.reason}</p></details>)}</div> : null}<p className="truth-note">{review.publish_allowed ? "审核已通过，公开资源允许展示。" : "审核未通过前，D 不展示 C 的讲义、代码实验或正式测评。"}</p></article>
}
...
function LessonPage({ user, onAssessment }: { user?: WorkspaceUser; onAssessment: () => void }) {
  ...
  const resourceRound = publishedResourceRound(activeSession)
  const showingPriorRound = resourceRound !== null && resourceRound < activeSession.round_no
  ...
  return <div className="lesson-page"><LearningLoopStepper session={activeSession} /><header className="lesson-topline"><div><span className="eyebrow"><BookOpen size={15} /> 第 {resourceRound ?? activeSession.round_no} 轮已发布学习资源</span>{showingPriorRound ? <span className="lesson-adaptation-badge">当前第 {activeSession.round_no} 轮生成受阻，可继续复习上一轮</span> : null}{adaptation ? <span className={`lesson-adaptation-badge is-${adaptation.adaptation_action}`}>{adaptation.adaptation_action === "remediate" ? "针对性补救" : adaptation.adaptation_action === "reinforce" ? "巩固强化" : "下一节点适配"}</span> : null}{user?.goalUseCase ? <span className="lesson-goal-badge"><Target size={13} /> {goalUseCaseLabel(user.goalUseCase)}导向</span> : null}<h1>{lesson?.title ?? "当前没有可发布的 C 讲义"}<button className="resource-match-trigger" type="button" onClick={() => setMatchOpen(true)} aria-label="查看本轮结构适配指数"><Sparkles size={14} />结构适配指数</button></h1><p>{activeSession.current_path_node?.node_id} · {lesson?.objective_ids.join(" / ")}</p>{user?.goalUseCase ? <div className="lesson-goal-note">本讲义按你的目标定制：{goalUseCaseDetail(user.goalUseCase)}</div> : null}{profileV2?.learning_preferences || profileV2?.progress ? <div className="lesson-profile-note"><Sparkles size={14} /><b>画像驱动本轮生成</b><span>{profileV2.learning_preferences?.explanation ? `讲解方式：${explanationPreferenceDetailLabel(profileV2.learning_preferences.explanation)}` : ""}{profileV2.learning_preferences?.practice ? ` · 练习方式：${practicePreferenceDetailLabel(profileV2.learning_preferences.practice)}` : ""}{profileV2.learning_preferences?.pace ? ` · 节奏：${pacePreferenceDetailLabel(profileV2.learning_preferences.pace)}` : ""}{profileV2.progress?.last_assessment_accuracy != null ? ` · 上轮准确率 ${Math.round(profileV2.progress.last_assessment_accuracy * 100)}%` : ""}</span></div> : null}{adaptation ? <div className="lesson-adaptation-note"><TrendingUp size={14} /><b>{showingPriorRound ? "上一轮适配" : "本轮适配"}：{adaptation.adaptation_action === "remediate" ? "针对性补救" : adaptation.adaptation_action === "reinforce" ? "巩固强化" : "下一节点适配"}</b><span>{adaptation.adaptation_summary}</span>{adaptation.addressed_misconception_tags.length ? <em>针对误区：{adaptation.addressed_misconception_tags.join("、")}</em> : null}</div> : null}</div><div className="lesson-top-actions"><span><CheckCircle2 size={15} /> 主 Agent已发布公开学习资源</span><button type="button" onClick={onAssessment}>进入正式测评 <ArrowRight /></button></div></header>
    ...
    <div className={`lesson-layout${tab === "lesson" ? "" : tab === "lab" ? " lab-fullwidth" : " lesson-layout-no-outline"}`}>
      ...
      <section className="lesson-main">
        {!lesson ? <EmptyState title="C 讲义尚未发布" body="D 不会生成占位知识。请等待主 Agent返回 learning_resources.concept_lesson。" /> : tab === "lesson" ? <LessonContent lesson={lesson} onActive={handleSectionScroll} onRunExample={runExampleCode} /> : tab === "lab" ? <LabContent lab={lab} code={code} setCode={setCode} gapAnswers={gapAnswers} setGapAnswers={setGapAnswers} busy={busy} execution={activeSession.code_execution?.labId === lab?.lab_id ? activeSession.code_execution : null} readOnly={showingPriorRound} onDebug={async (publicCaseId) => { if (!lab || !debugPublishedCodeLab || showingPriorRound) return; const submission = lab.programming_task?.submission_mode === "gap_answers" ? { gap_answers: gapAnswers } : code; await debugPublishedCodeLab(lab.lab_id, submission, publicCaseId) }} onRun={async () => { if (!lab || showingPriorRound) return; const submission = lab.programming_task?.submission_mode === "gap_answers" ? { gap_answers: gapAnswers } : code; await runPublishedCodeLab(lab.lab_id, submission); setLastExecutedCode(typeof submission === "string" ? submission : JSON.stringify(submission)) }} /> : <ChecksContent lesson={lesson} />}
      </section>
      {tab !== "lab" ? <aside className="lesson-side">{tab === "checks" ? <><div className="side-tabs"><button className={sideTab === "hint" ? "is-active" : ""} onClick={() => setSideTab("hint")} type="button">学习提示</button></div>{sideTab === "hint" ? <HintPanel resource={lesson} /> : null}</> : <><div className="side-tabs"><button className={sideTab === "evidence" ? "is-active" : ""} onClick={() => setSideTab("evidence")} type="button">知识来源</button><button className={sideTab === "agents" ? "is-active" : ""} onClick={() => setSideTab("agents")} type="button">Agent过程</button></div>{sideTab === "evidence" ? <EvidencePanel resource={lesson} /> : <AgentPanel />}</>}</aside> : null}
          </div>
...
function LabContent({ lab, code, setCode, gapAnswers, setGapAnswers, busy, execution, readOnly = false, onDebug, onRun }: { lab?: CodeLabPayload; code: string; setCode: (code: string) => void; gapAnswers: Record<string, string>; setGapAnswers: (answers: Record<string, string>) => void; busy: string; execution: PublicSessionFixture["code_execution"]; readOnly?: boolean; onDebug: (publicCaseId: string) => Promise<void>; onRun: () => Promise<void> }) {
  ...
  return <div className="lab-workspace">
    ...
    <section className="editor-panel">
      ...
      {readOnly ? <div className="gap-howto"><b>当前展示的是上一轮已审核实验</b><span>可继续查看题面、指南和提示；待本轮资源恢复后再调试或正式提交。</span></div> : null}
      {gapMode ? <div className="gap-editor">
        <div className="gap-howto"><b>只填写下面的输入框</b><span>不用复制整段代码。你填写后，系统会自动放入完整程序。</span></div>
      ...
      <footer><button type="button" onClick={() => gapMode ? setGapAnswers({}) : setCode(lab.starter_code)}><RotateCcw size={15} /> 重置</button><button className="secondary-action" disabled={readOnly || Boolean(busy) || !firstPublicCaseId || (gapMode ? !gapReady : !code.trim())} type="button" onClick={() => firstPublicCaseId && void onDebug(firstPublicCaseId)}><Play size={15} /> 调试公开样例</button><button className="run-button" disabled={readOnly || Boolean(busy) || (gapMode ? !gapReady : !code.trim())} type="button" onClick={() => void onRun()}><ShieldCheck size={15} /> {busy ? "评测中…" : "正式提交"}</button></footer>
