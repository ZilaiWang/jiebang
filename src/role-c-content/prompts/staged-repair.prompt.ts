...
若失败项包含 STDIN_FUNCTION_CONTRACT_MISMATCH：execution_mode 已由编排器冻结为 stdin_stdout，不得改为 function。必须删除 entry_point，starter 不得定义入口函数或使用 return 交付评分结果，任务、starter 和公开测试须统一为完整程序的标准输入/标准输出语义。不得删除证据要求的正常知识，例如“input() 返回字符串”并不等于要求学习者提交函数。不得混用两种模式。
