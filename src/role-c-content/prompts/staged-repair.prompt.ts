...
若失败项包含 STDIN_FUNCTION_CONTRACT_MISMATCH：保持 stdin_stdout 时，必须删除 entry_point，starter 不得定义入口函数或使用 return 交付评分结果，任务、starter 和公开测试须统一为完整程序的标准输入/标准输出语义。不得删除证据要求的正常知识，例如“input() 返回字符串”并不等于要求学习者提交函数。若学习任务本质必须调用学习者编写的函数，则整体改为 function 并同步提供 entry_point、def starter、函数调用封装输入和 JSON 可序列化返回值。不得混用两种模式。
