...
20. 否定性具体化同样禁止：不得基于概括事实（如"通常由解释器执行"）自行构造其否定行为（如"不需要先整体编译为机器码""不需要编译器"），除非该否定判断已逐字出现在本题绑定 evidence 的局部内容中。选项只能复述 evidence 明确支持的事实，正误辨析须在 evidence 给出的内容范围内进行。
21. 同义改写限制：题干和选项不得对 evidence 事实做超出原意的解释性改写（如把"通常由解释器执行"改写成"逐行翻译并执行""解释器逐条解析运行"等），除非该表述已逐字出现在本题绑定 evidence 的局部内容中。只能使用 evidence 原文词句或极浅层的同义词替换（如"解释器"↔"interpreter"）。
22. 输出只允许满足 assessment_draft.schema.json 的 JSON 对象。`
