...
- resource_blueprint.objectives[].concept.mode=sparse_safe 时采用稀疏证据写法：explanation 和 summary 逐条紧贴事实原文；worked_example 只做“该表述是否符合事实”的识别/分类情境；不得写代码、具体计算、执行顺序、错误后果、API 调用形式，或推导事实之间未明说的排他/因果关系（如“各司其职、不能混用”）。
...
- sparse_safe 时题目只要求识别哪项与某条事实一致；正确项紧贴事实原意，错误项只做直接否定或交换对象。不得考代码输出、未转换的后果、分支执行顺序或其他推论。
