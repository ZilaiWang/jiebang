...
`GenerationSpec` 是 C 的封闭终局合同。画像、路径和运行时对象先由 builder 按白名单投影为合同字段，调度元数据、恢复上下文和上游扩展字段不直接透传。服务启动时同时校验 TypeScript 字段表与 JSON Schema 的字段、必填项和枚举一致性，并在健康检查中公开合同版本、Schema 指纹和已加载数量；严格 Schema 因而只拒绝真实合同错误，不会把合法的上游扩展误当成生成失败。
...
代码实验的三级提示由模型结合当前任务、练习行为、事实文本和可见代码逐级生成，程序只检查三层互异、由方向到关键操作再到接近答案的渐进关系，以及至少两层包含当前事实或操作锚点。规范化过程保留合格的模型提示，不再用跨实验固定模板覆盖。
...
- 每个工作流共享总时限、模型调用数和传输重试预算；调用上限由目标数、测评题量、公开候选数、内部修复和外部修订轮数确定性计算；完整审核工作流的模型硬时限为 8 分钟，持久任务时限为 10 分钟，留出审核结果落盘和会话发布时间；认证、余额、权限和参数错误不重试，瞬时网络或服务拥堵只做一次带抖动重试；
...
C 的正式创作上下文只投影带 `source_id + fact_id` 的事实。知识库示例仅在其全部 `fact_refs` 都属于当前公开块的引用闭包时作为已审核实例一并交给语义审核，可用于支持该实例中实际展示的语法、过程和直接结果，但不能外推新的通用规则。没有事实身份的示例、练习和题目种子不进入可发表知识上下文。新名称、新数值和虚构对象可作为已引事实的直接实例，不能借实例补充新的 API、语法、返回类型或运行机制。
